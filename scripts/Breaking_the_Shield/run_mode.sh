...
case "${family}:${mode}" in
  abort-shake256-absorb-loop:baseline|abort-shake256-absorb-loop:attack|\
  skip-one-shake256-absorb-block:baseline|skip-one-shake256-absorb-block:attack|\
  polyz-unpack-zero-load:baseline|polyz-unpack-zero-load:attack|\
  polyz-unpack-stale-load:baseline|polyz-unpack-stale-load:attack) ;;
  *) echo "[error] invalid family/mode: ${family}/${mode}" >&2; exit 2 ;;
esac
counter_set=""
for index in "${!BTS_PASSES[@]}"; do
  if [[ "${BTS_PASSES[$index]}" == "$pass" ]]; then
    counter_set="$((index + 1))"
    break
  fi
done
...
if [[ -n "${BTS_CPU_CORE:-}" ]]; then
  cpu_core="$(python3 "${BTS_SCRIPT_DIR}/resolve_cpu.py" --cpu "${BTS_CPU_CORE}" --report-output "${BTS_RESULTS_ROOT}/cpu_affinity.json")"
else
  cpu_core="$(python3 "${BTS_SCRIPT_DIR}/resolve_cpu.py" --report-output "${BTS_RESULTS_ROOT}/cpu_affinity.json")"
fi
out_dir="${BTS_RESULTS_ROOT}/${family}/${pass}"
mkdir -p "$out_dir"
output="${out_dir}/${stem}.csv"
"$binary" --samples "$samples" --warmup "${BTS_WARMUP:-20}" \
