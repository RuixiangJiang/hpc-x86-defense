...
EVENT_SPECS = {
    "L1D_REPLACEMENTS": {"candidates": ["l1d.replacement", "l1d.replacements"]},
    ...
    "STALLS_MEM_ANY": {
        "candidates": ["cycle_activity.stalls_mem_any", "cycle_activity.stalls_total"]
    },
    ...
    "RECOVERY_CYCLES_ANY": {"candidates": ["int_misc.recovery_cycles_any"]},
}
...
def write_header(path: Path, resolved: dict[str, Encoding]) -> None:
    lines = [
        "#ifndef MFK_MICROARCH_EVENTS_GENERATED_H",
        "#define MFK_MICROARCH_EVENTS_GENERATED_H",
        "",
    ...
    for name, item in resolved.items():
        lines.extend([
            f"#define MFK_EVT_{name}_AVAILABLE {1 if item.available else 0}",
            f"#define MFK_EVT_{name}_TYPE {item.type}u",
            f"#define MFK_EVT_{name}_CONFIG {c_u64(item.config)}",
            f"#define MFK_EVT_{name}_CONFIG1 {c_u64(item.config1)}",
            f"#define MFK_EVT_{name}_CONFIG2 {c_u64(item.config2)}",
            f"#define MFK_EVT_{name}_PRECISE_IP {item.precise_ip}u",
            "",
        ])
    lines.extend(["#endif", ""])
    content = "\n".join(lines)
