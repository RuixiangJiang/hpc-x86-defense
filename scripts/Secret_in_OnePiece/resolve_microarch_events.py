...
def write_header(path: Path, resolved: dict[str, Encoding]) -> None:
    lines = [
        "#ifndef SIO_MICROARCH_EVENTS_GENERATED_H",
        "#define SIO_MICROARCH_EVENTS_GENERATED_H",
        "",
    ...
    for name, item in resolved.items():
        lines.extend([
            f"#define SIO_EVT_{name}_AVAILABLE {1 if item.available else 0}",
            f"#define SIO_EVT_{name}_TYPE {item.type}u",
            f"#define SIO_EVT_{name}_CONFIG {c_u64(item.config)}",
            f"#define SIO_EVT_{name}_CONFIG1 {c_u64(item.config1)}",
            f"#define SIO_EVT_{name}_CONFIG2 {c_u64(item.config2)}",
            f"#define SIO_EVT_{name}_PRECISE_IP {item.precise_ip}u",
            "",
        ])
    lines.extend(["#endif", ""])
    content = "\n".join(lines)
