...
case "$MODE" in
    baseline)
        BIN="$BUILD_DIR/bin/delvaux_roulette/roulette_baseline"
        ;;
    skip-add)
        BIN="$BUILD_DIR/bin/delvaux_roulette/roulette_skip_add"
        ;;
    *)
...
if [[ ! "$ROULETTE_TARGET_COEFF" =~ ^[0-9]+$ ]] ||
   (( ROULETTE_TARGET_COEFF < 0 || ROULETTE_TARGET_COEFF >= 128 )); then
    echo "[error] ROULETTE_TARGET_COEFF must be in [0, 128)" >&2
    exit 1
fi
...
make -C "$REPO_ROOT" roulette
mkdir -p "$EXP_RESULTS_DIR"

KEY_FILE="$EXP_RESULTS_DIR/kyber768.key"
LOG_FILE="${OUTPUT%.csv}.log"

ARGS=(
    --samples "$ROULETTE_SAMPLES"
    --warmup "$ROULETTE_WARMUP"
    --target-coeff "$ROULETTE_TARGET_COEFF"
    --seed "$ROULETTE_SEED"
    --key-file "$KEY_FILE"
    --output "$OUTPUT"
)
...
if [[ "$CREATE_KEY" == "--create-key" ]]; then
    ...
    echo "[hint] run the baseline calibration first" >&2
    exit 1
fi

echo "[configuration]"
echo "  paper:         Delvaux Roulette"
echo "  mode:          $MODE"
echo "  Kyber:         Kyber768"
echo "  CPU:           $HPC_CPU"
echo "  cpu_core CPUs: ${CORE_CPUS:-unavailable}"
echo "  cpu_atom CPUs: ${ATOM_CPUS:-unavailable}"
echo "  samples:       $ROULETTE_SAMPLES"
echo "  warmup:        $ROULETTE_WARMUP"
echo "  target coeff:  $ROULETTE_TARGET_COEFF"
echo "  mask seed:     $ROULETTE_SEED"
echo
