...
echo "[1/6] build and verify exact target assembly"
"$SCRIPT_DIR/verify_target_assembly.sh"

echo "[2/6] remove prior named FPR datasets"
rm -f \
    ...
    "$RESULT_DIR/attack_skip_add.csv" \
    "$RESULT_DIR/attack_skip_add.log" \
    "$RESULT_DIR/detector_model.json" \
    "$RESULT_DIR/fpr_tpr_report.json" \
    "$RESULT_DIR/fpr_tpr_report.txt"

echo "[3/6] collect 500 baseline calibration samples"
"$SCRIPT_DIR/run_fpr_calibration_baseline.sh"

echo "[4/6] collect independent 5000 baseline validation samples"
"$SCRIPT_DIR/run_fpr_validation_baseline.sh"

echo "[5/6] collect 500 skipped-share-addition attack samples"
"$SCRIPT_DIR/run_fpr_attack_skip_add.sh"

echo "[6/6] freeze detector and evaluate FPR/TPR"
python3 "$SCRIPT_DIR/analyze_fpr_tpr.py" \
    ...
    --attack "$RESULT_DIR/attack_skip_add.csv" \
